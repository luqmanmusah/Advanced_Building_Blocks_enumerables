module Enumerable
  ...
  def my_all?(*args)
    ...
    if args.length == 1 && args.first.instance_of?(Regexp)
      my_all_booleam = true
      each do |i|
        my_all_booleam = false unless args.first.match(i)
      end
      return my_all_booleam
    end
    ...
    if args.length == 1
      ...
      if args.first.is_a?(Class)
        my_all_booleam = true
        each do |i|
          my_all_booleam = false unless i.is_a?(args.first)
        end
        return my_all_booleam
      end
      ...
      my_all_booleam = true
      each do |i|
        my_all_booleam = false unless i == args.first
      end
      return my_all_booleam
    end

    # Check for when a block is not given
    unless block_given?
      my_all_booleam = true
      each do |i|
        my_all_booleam = false unless i
      end
      return my_all_booleam
    end
    ...
    my_all_booleam = true
    each do |i|
      my_all_booleam = false unless yield(i)
    end
    my_all_booleam
  end
